/**
 * ═══════════════════════════════════════════════════════════════
 *   APP Panel  ·  Enterprise Proxy Panel  v3.5 (Full Brain & Ping)
 *   اسکنر ۵۰ آی‌پی + تست پینگ مرورگر + اضافه کردن سریع‌ترین‌ها
 * ═══════════════════════════════════════════════════════════════
 */
...
async function handleAPI(request, env, path) {
  ...
  if (path === '/api/brain') {
    ...
    const CF_RANGES = [
      [104,16,0,0, 104,23,255,255], [104,24,0,0, 104,27,255,255], [172,64,0,0, 172,71,255,255], [162,158,0,0, 162,159,255,255], [188,114,96,0, 188,114,111,255], [141,101,64,0, 141,101,127,255], [190,93,240,0, 190,93,255,255], [198,41,128,0, 198,41,255,255], [173,245,48,0, 173,245,63,255], [108,162,192,0, 108,162,255,255]
    ];
    ...
    if (mode === 'scan') {
      ...
      while (items.length < n && guard < n * 5) {
        ...
        seen.add(ip);
        items.push({ ip, tag: 'scan', ms: 0, ok: true });
      }
      return json({ ips: items });
    }

    return json({
      ips: CURATED.map((x, i) => ({ ...x, ms: 0, ok: true, type: x.tag }))
    });
  }
