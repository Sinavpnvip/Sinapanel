/**
 * ═══════════════════════════════════════════════════════════════
 *   APP Panel  ·  Enterprise Proxy Panel  v3.3 (Bulletproof DB)
...
async function handleAPI(request, env, path) {
  ...
  if (path === '/api/brain') {
    // Curated high-quality Cloudflare edges & domains
    const CURATED = [
      {ip:'cdnjs.cloudflare.com', tag:'domain'},
      {ip:'cloudflare.com', tag:'domain'},
      {ip:'www.cloudflare.com', tag:'domain'},
      {ip:'speed.cloudflare.com', tag:'domain'},
      {ip:'workers.cloudflare.com', tag:'domain'},
      {ip:'cloudflare-dns.com', tag:'domain'},
      {ip:'one.one.one.one', tag:'domain'},
      {ip:'ajax.cloudflare.com', tag:'domain'},
      {ip:'104.16.128.50', tag:'cf'},
      {ip:'104.16.147.117', tag:'cf'},
      {ip:'104.16.80.73', tag:'cf'},
      {ip:'104.16.89.120', tag:'cf'},
      {ip:'104.17.176.20', tag:'cf'},
      {ip:'104.17.222.48', tag:'cf'},
      {ip:'104.18.22.100', tag:'cf'},
      {ip:'104.18.79.186', tag:'cf'},
      {ip:'104.18.139.67', tag:'cf'},
      {ip:'104.19.229.21', tag:'cf'},
      {ip:'104.19.250.20', tag:'cf'},
      {ip:'104.21.48.10', tag:'cf'},
      {ip:'104.21.19.124', tag:'cf'},
      {ip:'104.22.11.129', tag:'cf'},
      {ip:'104.22.75.105', tag:'cf'},
      {ip:'104.24.62.187', tag:'cf'},
      {ip:'104.25.134.44', tag:'cf'},
      {ip:'162.159.1.94', tag:'cf'},
      {ip:'162.159.193.84', tag:'cf'},
      {ip:'162.159.32.35', tag:'cf'},
      {ip:'172.67.158.243', tag:'cf'},
      {ip:'172.67.186.42', tag:'cf'},
      {ip:'172.67.229.179', tag:'cf'},
      {ip:'172.64.35.226', tag:'cf'},
      {ip:'188.114.96.156', tag:'cf'},
      {ip:'188.114.99.178', tag:'cf'},
      {ip:'190.93.245.251', tag:'cf'},
      {ip:'141.101.115.22', tag:'cf'},
      {ip:'198.41.202.5', tag:'cf'},
      {ip:'104.16.6.65', tag:'cf'},
      {ip:'104.16.117.43', tag:'cf'},
      {ip:'104.16.63.25', tag:'cf'},
      {ip:'104.16.7.70', tag:'cf'},
      {ip:'104.27.62.243', tag:'cf'},
      {ip:'172.65.13.222', tag:'cf'},
      {ip:'104.20.224.95', tag:'cf'},
      {ip:'162.159.250.246', tag:'cf'},
      {ip:'104.18.220.84', tag:'cf'}
    ];

    // Official CF ranges for random scan
    const CF_RANGES = [
    ...
    // Allowed CF HTTPS ports commonly used in panels
    const ALLOWED_PORTS = [443, 2053, 2083, 2087, 2096, 8443];
    let testPort = parseInt(url.searchParams.get('port') || '443', 10) || 443;
    if (!ALLOWED_PORTS.includes(testPort)) testPort = 443;

    function randInt(a, b) { return a + Math.floor(Math.random() * (b - a + 1)); }
    function randomCFIP() {
      const r = CF_RANGES[randInt(0, CF_RANGES.length - 1)];
      const a = randInt(r[0], r[4]);
      const b = (a === r[0] && a === r[4]) ? randInt(r[1], r[5]) : (a === r[0] ? randInt(r[1], 255) : (a === r[4] ? randInt(0, r[5]) : randInt(0, 255)));
    ...
    async function probeTCP(host, port) {
      ...
      try {
        ...
        const opened = sock.opened ? sock.opened : (async () => { const w = sock.writable.getWriter(); w.releaseLock(); })();
    ...
    async function probeList(items, port) {
      const results = [];
      for (let i = 0; i < items.length; i += 8) {
        const batch = items.slice(i, i + 8);
        const part = await Promise.all(batch.map(async (item) => {
          const r = await probeTCP(item.ip, port);
          return { ip: item.ip, tag: item.tag || 'cf', ms: r.ms, ok: r.ok, port };
        }));
        results.push(...part);
      }
      results.sort((a, b) => (a.ms < 0 ? 99999 : a.ms) - (b.ms < 0 ? 99999 : b.ms));
      return results;
    }

    const mode = url.searchParams.get('mode') || 'curated';
    const wantTest = url.searchParams.get('test') === '1' || mode === 'test';
    const wantScan = mode === 'scan' || url.searchParams.get('scan') === '1';

    if (wantScan) {
      const n = Math.min(parseInt(url.searchParams.get('n') || '50', 10) || 50, 80);
      const seen = new Set();
      const items = [];
      let guard = 0;
      while (items.length < n && guard < n * 5) {
        guard++;
        const ip = randomCFIP();
        if (seen.has(ip)) continue;
        seen.add(ip);
        items.push({ ip, tag: 'scan' });
      }
      const results = await probeList(items, testPort);
      const fast = results.filter(r => r.ok && r.ms > 0 && r.ms < 100).length;
      return json({ ips: results, tested: true, mode: 'scan', count: results.length, fast, port: testPort, ports: ALLOWED_PORTS, note: 'Random scan on port ' + testPort });
    }

    if (wantTest) {
      const results = await probeList(CURATED, testPort);
      const fast = results.filter(r => r.ok && r.ms > 0 && r.ms < 100).length;
      return json({ ips: results, tested: true, mode: 'curated', count: results.length, fast, port: testPort, ports: ALLOWED_PORTS, note: 'Curated TCP:' + testPort });
    }

    return json({
      ips: CURATED.map((x, i) => ({ ...x, ms: x.tag === 'domain' ? 25 + (i % 8) : 40 + (i % 30), ok: true, port: testPort })),
      tested: false, mode: 'curated', count: CURATED.length, port: testPort, ports: ALLOWED_PORTS
    });
  }
